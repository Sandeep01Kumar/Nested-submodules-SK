...
// EXPRESSION (not a top-level `function subtract` declaration) so the browser
// C-002 bridge — which loads these files as classic <script>s — does not also
// leak a stray `window.subtract` global (F-SEC-01 namespace pollution). A named
...
module.exports = function subtract(a,b){
    ...
    // non-finite value (e.g. subtract(Number.MAX_VALUE, -Number.MAX_VALUE) === Infinity).
